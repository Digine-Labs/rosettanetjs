...
function extractNextParameter(functionParameters: string): {
...
function toFullParameter(parameter: ParameterDescription): string {
  if (parameter.type.startsWith('tuple')) {
    if (parameter.components === undefined)
      throw new Error(
        `Encountered a 'tuple' type that had no components.  Did you mean to include an empty array?`
      );
...
export async function encodeMethod(
  keccak256: (message: Uint8Array) => Promise<bigint>,
  functionDescription: EVMFunction,
  parameters: EncodableArray
): Promise<Uint8Array>;
export async function encodeMethod(
  keccak256: (message: Uint8Array) => Promise<bigint>,
...
export function encodeMethod(
  ...
  parameterDescriptions: ReadonlyArray<ParameterDescription>,
  parameters: EncodableArray
): Uint8Array;
export function encodeMethod(
  first: ((message: Uint8Array) => Promise<bigint>) | number,
  second: EVMFunction | string | ReadonlyArray<ParameterDescription> | EncodableArray,
  parameters: EncodableArray
): Promise<Uint8Array> | Uint8Array {
  if (typeof first === 'number')
    return encodeMethodWithSelector(
      first,
      second as ReadonlyArray<ParameterDescription>,
      parameters
    );
...
async function encodeMethodWithDescription(
  keccak256: (message: Uint8Array) => Promise<bigint>,
  functionDescription: EVMFunction,
  parameters: EncodableArray
): Promise<Uint8Array> {
...
async function encodeMethodWithSignature(
  keccak256: (message: Uint8Array) => Promise<bigint>,
  functionSignature: string,
  parameters: EncodableArray
): Promise<Uint8Array> {
...
function encodeMethodWithSelector(
  functionSelector: number,
  parameterDescriptions: ReadonlyArray<ParameterDescription>,
  parameters: EncodableArray
): Uint8Array {
...
export function encodeParameters(
  descriptions: ReadonlyArray<ParameterDescription>,
  parameters: EncodableArray
): Uint8Array {
  ...
  const encodedParameters = parameters.map((nestedParameter, index) =>
    encodeParameter(descriptions[index], nestedParameter)
  );
...
function encodeParameter(
  ...
  parameter: Encodable
): { isDynamic: boolean; bytes: Uint8Array } {
  return (
    tryEncodeFixedArray(description, parameter) ||
...
function tryEncodeFixedArray(
  description: ParameterDescription,
  parameter: Encodable
): { isDynamic: boolean; bytes: Uint8Array } | null {
  ...
  const encodedParameters = parameter.map((nestedParameter) =>
    encodeParameter(nestedDescription, nestedParameter)
  );
...
function tryEncodeDynamicArray(
  description: ParameterDescription,
  parameter: Encodable
): { isDynamic: true; bytes: Uint8Array } | null {
...
function tryEncodeTuple(
  ...
  if (description.components === undefined || description.components.length === 0) {
    ...
    const encodedComponents = description.components.map((component, index) => {
      const parameter = isEncodableArray(encodableTupleOrArray)
        ? encodableTupleOrArray[index]
        : encodableTupleOrArray[component.name];
    ...
    return {
      isDynamic: isDynamic,
      bytes: isDynamic
        ? encodeDynamicData(encodedComponents)
        : concatenateBytes(encodedComponents.map((x) => x.bytes)),
    };
  }
}

function tryEncodeDynamicBytes(
  description: ParameterDescription,
  parameter: Encodable
): { isDynamic: true; bytes: Uint8Array } | null {
...
function tryEncodeBoolean(
  description: ParameterDescription,
  parameter: Encodable
): { isDynamic: false; bytes: Uint8Array } | null {
...
function tryEncodeFixedPointNumber(
  description: ParameterDescription
): { isDynamic: never; bytes: Uint8Array } | null {
